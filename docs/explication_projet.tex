...
Cross-dataset      & HAPT$\to$WISDM~: 55\% rétention \\
...
La vraie cause~: trop de tâches.
\begin{itemize}
  \item 2 classes/tâche $\to$ 6 tâches $\to$ F1 = 0.16
  \item 6 classes/tâche $\to$ 2 tâches $\to$ F1 = \textbf{0.40}
\end{itemize}
Avec 2 classes par tâche, le modèle traverse 6 phases de reconfiguration
successives. La solution~: augmenter le nombre de classes par tâche.
\end{boiteverte}
...
\midrule
HAPT $\to$ HAPT (référence)    & 0.838 & 100\% \\
HAPT $\to$ WISDM (autre appareil) & 0.464 & \textbf{55\%} \\
HAPT $\to$ PAMAP2 (placement diff.) & 0.124 & 15\% \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Interprétation~:} le backbone transfère bien entre HAPT et WISDM
(même port, fréquences différentes~: 55\% de rétention sans adaptation).
La chute vers PAMAP2 (15\%) est due au changement de placement du capteur
(poitrine vs poche) --- du domain shift structurel.

\begin{boitegrise}
\textbf{Solution~:} quelques exemples du nouveau domaine suffisent
(Demo 2 le prouve~: 100 fenêtres $\to$ 90\% de précision sur le nouveau sujet).
\end{boitegrise}
...
& $p=25\%$ & $p=50\%$ & $p=75\%$ \\
\midrule
Backbone gelé (frozen) & 0.093 & 0.093 & 0.094 \\
Entraînement conjoint  & 0.091 & 0.091 & 0.084 \\
Ligne de base majoritaire & \multicolumn{3}{c}{0.305} \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Piste future~:} backbone pré-entraîné spécifiquement pour la
prédiction temporelle (contrastive predictive coding) ou entraînement
multi-tâche avec perte pondérée dès le départ.
\end{boiteorange}
...
\midrule
Cross-dataset           & HAPT$\to$WISDM  & 55\% rétention \\
                        & HAPT$\to$PAMAP2 & 15\% (domain shift) \\
\midrule
Anticipation            & Frozen / Joint  & F1 = 0.094 / 0.091 \\
                        & Ligne de base   & 0.305 (majoritaire) \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
  ...
  \item La généralisation cross-dataset est partielle (55\% WISDM)
        mais rapidement récupérable avec quelques exemples.
...
\begin{enumerate}
  ...
  \item Adaptateur de domaine pour le cross-dataset (PAMAP2)
  \item Extension à MobiAct (chutes) et données non étiquetées (SSL)
\end{enumerate}
