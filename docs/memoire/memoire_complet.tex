...
fiche PFE~26/2755. Après un état de l'art, nous décrivons un backbone Transformer
partagé, la reconnaissance continue avec Uncertainty-Weighted Replay (UWR),
l'anticipation multi-classe (tête LSTM sur fenêtres partielles), plus des modules
complémentaires : SSL SimCLR-style, pipeline foundation-style, calibration de
confiance en ligne, bandit de seuil (RL léger) et fall-risk binaire. L'évaluation
sur HAPT et WISDM montre des gains nets par rapport à EWC, iCaRL et finetuning
naïf (+16,8 points macro-F1 en user-incrémental). L'anticipation multi-classe
atteint un macro-F1 de validation d'environ 0,60--0,64 (baseline majorité
$\approx$ 0,04) ; le fall-risk binaire atteint environ 0,88.
...
calibration, fall-risk et bandit de seuil. Les expériences couvrent le
pré-entraînement (91,3~\% accuracy HAPT), l'adaptation 44 sujets (F1 0,543),
le class-incrémental six tâches, l'anticipation LSTM (macro-F1 $\approx$0,60--0,64),
le fall-risk binaire ($\approx$0,88), et l'adaptation cross-dataset CORAL
...
fiches bibliographiques pour faciliter la reproduction.
...
This thesis presents an adaptive continual HAR architecture: a shared Transformer
backbone, continual recognition with Uncertainty-Weighted Replay (UWR), and
multi-class next-activity anticipation from partial windows (LSTM head, frozen
backbone). Experiments on HAPT and WISDM show clear gains over EWC, iCaRL and
naive fine-tuning (+16.8 macro-F1 points in user-incremental learning). Multi-class
anticipation reaches validation macro-F1 $\approx$ 0.60--0.64 (majority baseline
$\approx$ 0.04); binary fall-risk reaches $\approx$ 0.88. Complementary modules
cover SSL, foundation-style pretraining, online calibration and a light threshold
bandit (RL).
...
\noindent
\textarabic{%
يقترح هذا المذكّر بنية HAR تكيفية ومستمرة: عمود فقري Transformer،
تعرف مستمر بـ UWR، وتوقع متعدد الأصناف عبر رأس LSTM على نوافذ جزئية.
التجارب على HAPT و WISDM تظهر مكاسب واضحة مقارنة بـ EWC و iCaRL والضبط البسيط.}
...
tête LSTM sur fenêtres partielles, plus des modules CORAL, SSL,
foundation-style, calibration, bandit de seuil et fall-risk.
...
Le macro-F1 de validation atteint environ 0,60--0,64 selon $p$ (baseline majorité $\approx$ 0,04).
...
\begin{enumerate}
  ...
  adaptateur CORAL, fall-risk, calibration online, bandit de seuil,
  SSL / foundation-style.
\end{enumerate}
...
\subsection{Incertitude et Monte Carlo Dropout}

Pour prioriser les exemples difficiles en replay (UWR, chapitre~4), nous
estimons l'incertitude épistémique par Monte Carlo Dropout
\parencite{gal2016} : plusieurs passes forward avec dropout actif, puis entropie
de la distribution moyenne des probabilités. Gal et Ghahramani distinguent
incertitude épistémique (modèle) et aléatoire (données) ; seule la première
nous intéresse pour guider la mémoire sous contrainte embarquée.
...
une perte contrastive et un score d'incertitude (MC Dropout) pour pondérer les
...
% ===== BEGIN input: includes/tikz-har-system.tex =====
%% Vue d'ensemble du système HARContinualModel (har_project)
\begin{figure}[H]
\centering
\begin{tikzpicture}[
  node distance=0.55cm and 0.9cm,
  comp/.style={rectangle,rounded corners=3pt,draw=harblue!80!black,
    fill=harlightblue!40,minimum width=2.6cm,minimum height=0.85cm,
    align=center,font=\small\bfseries},
  mod/.style={rectangle,rounded corners=3pt,draw=hargreen!70!black,
    fill=harlightgreen!35,minimum width=2.5cm,minimum height=0.75cm,
    align=center,font=\footnotesize},
  arr/.style={-{Stealth[length=5pt]},line width=0.9pt,draw=harblue!70!black}
]
  \node[comp] (imu) {Capteurs\\IMU};
  \node[comp,right=of imu] (prep) {Prétraitement\\(50 Hz, fenêtre 150)};
  \node[comp,right=of prep] (bb) {Backbone\\Transformer};
  \node[mod, below=0.65cm of bb,xshift=-1.6cm] (cl) {Module 1\\HAR + UWR};
  \node[mod, below=0.65cm of bb,xshift=1.6cm] (ant) {Module 2\\Anticipation};
  \node[mod, below=0.55cm of cl] (fall) {Chutes\\hybride};
  \node[mod, below=0.55cm of ant] (coral) {Adaptateur\\CORAL};
  \draw[arr] (imu) -- (prep);
  \draw[arr] (prep) -- (bb);
  \draw[arr] (bb) -- (cl);
  \draw[arr] (bb) -- (ant);
  ...
  \draw[arr] (bb) -- (coral);
\end{tikzpicture}
\caption[Architecture globale du système proposé]{Architecture globale du système proposé :
backbone Transformer partagé, module de reconnaissance continue (UWR),
module d'anticipation LSTM, détection de chutes et adaptation CORAL
(\texttt{har\_project/src/models/har\_model.py}).}
\label{fig:system_overview}
\end{figure}

% ===== END input: includes/tikz-har-system.tex =====
...
\subsection{Calibration adaptative en ligne}
...
\begin{itemize}
  ...
  $X[i-S],\ldots,X[i-1]$, chacune tronquée aux \emph{derniers} $p\,\%$
  ($p\in\{0{,}25,0{,}50,0{,}75\}$) — les indices de transition étant souvent en fin de fenêtre ;
  ...
  multi-classe) ;
  \item la perte est la cross-entropie pondérée par classe ; on restaure le meilleur checkpoint (macro-F1 val).
\end{itemize}
...
La fiche PFE (code~26/2755) mentionne l'auto-supervision, les modèles de type
fondation, la calibration en ligne, l'apprentissage séquentiel / renforcement,
l'anticipation d'une perte d'équilibre et des activités domestiques (repas).
Les modules suivants couvrent ces orientations, avec des limites assumées.

\subsection{Pré-entraînement auto-supervisé (SSL)}

Un pré-entraînement SimCLR-style sur fenêtres IMU (sans labels) produit deux
vues augmentées (bruit, scale, masquage) et minimise une perte NT-Xent. Le
backbone (\texttt{ssl\_backbone.pt}) sert d'initialisation optionnelle — SSL
pragmatique, pas massif \parencite{yuan2024}.

\subsection{Pipeline foundation-style}

Motif prétrain générique $\rightarrow$ adaptation ciblée
(\texttt{train\_foundation\_style.py} $\rightarrow$ \texttt{foundation\_style.pt}),
sans intégrer un FM public. Le checkpoint de production reste
\texttt{pretrained.pt}.

\subsection{Calibration de confiance en ligne}

\texttt{OnlineCalibrator} combine température $T$ apprise en ligne (NLL) et
seuil d'acceptation adaptatif — distinct de la seule mise à jour des prototypes.

\subsection{Bandit de seuil (RL léger)}

Bandit $\varepsilon$-greedy (\texttt{ThresholdBandit}) sur une grille de seuils
d'alerte : récompense les vraies alertes, pénalise les fausses. Ce n'est pas
du deep RL.

\subsection{Anticipation du risque de chute / équilibre}

\texttt{FallRiskHead} prédit si la prochaine activité est une transition
posturale à risque (labels 9--12). \texttt{PhysicalFallDetector} complète la
couche sécurité. Pas de classe \texttt{fall} clinique dans le merge actuel.

\subsection{Scénario « préparation de repas »}

Documenté dans \texttt{adl\_scenarios.py} comme \emph{unavailable} (pas de
labels cuisine) ; proxy UI uniquement pour la démo Streamlit.
...
\texttt{transitions\_only} + fin de fenêtre (notre méthode) & $\approx$ 0,59--0,64 \\
\hline
\end{tabularx}
...
\section{Module auxiliaire — détection de chutes}

\subsection{Motivation}

La détection de chutes est un cas d'usage HAR à fort enjeu (aide à domicile).
Notre architecture modulaire permet d'ajouter un détecteur sans modifier le
backbone CL. En l'absence de MobiAct au moment des runs, nous utilisons les
transitions posturales brusques HAPT comme proxy positif et jogging comme négatif
difficile.
...
\begin{itemize}
  \item \textbf{Règles physiques} : magnitude acc $> 2{,}5g$ ET gyro $> 200$~deg/s
  sur la fenêtre 3~s — détecte les impacts brusques avec faible coût CPU.
  \item \textbf{MLP sur embedding} : 128 $\rightarrow$ 64 $\rightarrow$ 32 $\rightarrow$ 1,
  entraîné sur embeddings backbone gelé — capture des patterns subtils (différence
  chute vs saut volontaire).
\end{itemize}

\subsection{Métriques cibles}

AUC-ROC et F1 ; objectif clinique AUC $> 0{,}90$ sur MobiAct
\parencite{vavoulas2016}. Résultat actuel : AUC 0,762 (proxy) — preuve de concept
seulement.
...
atteint un macro-F1 de validation d'environ 0,60--0,64 selon le ratio d'observation (baseline majorité $\approx$ 0,04).
...
avec labels cibles partiels ; (5)~protocoles CL user (sujets triés) et class
($2$ classes/tâche). L'évaluation chapitre~6 valide ces choix.
...
|   |   |-- augmentation.py        # augmentation en ligne
...
Le suivi de version est géré par Git. Chaque composant fonctionnel (backbone, UWR, CORAL, anticipation) fait l'objet d'une implémentation et d'une validation indépendantes avant intégration dans le modèle complet.
...
\texttt{augmentation.py} lorsque le script l'active.
...
Augmentation. Lors du pré-entraînement, l'IMUAugmenter est activé avec probabilité $p_{\mathrm{apply}} = 0{,}5$ par transformation : bruit gaussien ($\sigma = 0{,}01$), mise à l'échelle ($s \in [0{,}85, 1{,}15]$), déformation temporelle ($\sigma_{\mathrm{warp}} = 0{,}05$), dropout de canaux ($p_{\mathrm{drop}} = 0{,}1$), et découpage de fenêtre ($r_{\mathrm{crop}} = 0{,}9$). L'augmentation permet d'améliorer l'accuracy de validation de 81~\% (sans) à 91,3~\% (avec), soit un gain de +10 points.
...
\subsection{Intégration du Monte Carlo Dropout}

Le Monte Carlo Dropout est implémenté dans src/models/mc\_dropout.py. La fonction add\_dropout\_to\_backbone(backbone, p=0.1) ajoute un module nn.Dropout supplémentaire au sein de chaque bloc Transformer, de manière non-destructive. Cette approche évite de modifier l'architecture du backbone déjà pré-entraîné.

\begin{lstlisting}[language=Python,caption={Estimation d'incertitude par MC Dropout},label=lst:mc_dropout]
def mc_uncertainty(model, x, n_passes=20):
...
La classe MCDropoutReplayBuffer encapsule un ReplayBuffer standard et un mécanisme d'appel à mc\_uncertainty lors de l'ajout de nouveaux exemples. Pour limiter le surcoût computationnel, l'estimation d'incertitude n'est effectuée que pour les exemples candidats à l'insertion, pas pour l'intégralité du tampon.
...
\texttt{AnticipationDataset} : contexte $S=5$ fenêtres tronquées en
\emph{fin} de fenêtre (\texttt{truncate\_from=end}), cible = label de la
fenêtre suivante, filtre \texttt{transitions\_only}. Le split train/val
est \textbf{par sujet} (\texttt{build\_anticipation\_datasets}).

Dans \texttt{anticipation\_trainer.py} : backbone + HAR gelés ; AdamW sur la
tête LSTM ; \texttt{cross\_entropy} pondérée par classe ; restauration du
meilleur macro-F1 de validation ; un entraînement par ratio $p$.
...
\subsection{Détection de chute — approche hybride}
...
ajoutés : pré-entraînement auto-supervisé SimCLR-style
(\texttt{scripts/train\_ssl.py} $\rightarrow$ \texttt{ssl\_backbone.pt}) ;
pipeline foundation-style SSL puis fine-tune
(\texttt{train\_foundation\_style.py} $\rightarrow$ \texttt{foundation\_style.pt},
run court démonstratif, distinct du \texttt{pretrained.pt} de production) ;
calibration online et bandit de seuil
(\texttt{online\_calibration.py}, \texttt{rl\_threshold\_agent.py}).
Le scénario « préparation de repas » reste hors portée faute de corpus cuisine.
L'interface Streamlit expose six scénarios (reconnaissance, nouvel utilisateur,
anticipation, fall-risk, calibration+RL, SSL/foundation).

\subsection{Adaptateur de domaine CORAL}

L'adaptation de domaine est implémentée dans src/models/domain\_adapter.py via la classe CORALAdapter. CORAL (CORrelation ALignment, Sun \& Saenko, 2016) aligne les statistiques de second ordre (matrices de covariance) entre les embeddings source et cible pour réduire le domain shift.

Calcul de la perte CORAL. La perte CORAL est définie comme la distance de Frobenius entre les matrices de covariance source $C_s$ et cible $C_t$ :

$$\mathcal{L}_{CORAL} = \frac{1}{4d^2} |C_s - C_t|_F^2$$

où $d = 128$ est la dimension des embeddings. La fonction coral\_loss(source\_features, target\_features) est implémentée comme suit :

def coral\_loss(source, target):

d = source.size(1)

\# Centrer les features
...
\# Distance de Frobenius normalisée

loss = torch.norm(Cs - Ct, p='fro') ** 2 / (4 * d * d)

return loss

CORAL non supervisé. Sans étiquettes cibles, seul l'alignement de covariance est appliqué. Le modèle source est fine-tuné sur les données cibles en minimisant $\mathcal{L}_{CORAL}$ seul, sans supervision. Les paramètres du backbone sont légèrement ajustés (lr = $10^{-5}$) tandis que la tête de classification reste gelée.

CORAL supervisé. Avec 20 \% des étiquettes cibles disponibles, la perte totale combine l'alignement de covariance et la supervision par cross-entropie :

$$\mathcal{L}_{\mathrm{total}} = \mathcal{L}_{\mathrm{CE}}(x_{\mathrm{cible, labeled}}) + \lambda_{\mathrm{CORAL}} \cdot \mathcal{L}_{\mathrm{CORAL}}(x_{\mathrm{source}}, x_{\mathrm{cible}})$$

avec $\lambda_{CORAL} = 1{,}0$. Le fine-tuning est effectué sur 20 epochs avec le backbone pré-entraîné sur HAPT.

\begin{table}[H]
\centering
\small
\begin{tabularx}{0.72\textwidth}{|Y|c|}
\hline
\textbf{Configuration CORAL} & \textbf{F1 macro} \\
\hline
Sans adaptation & 0,040 \\
CORAL non supervisé & 0,040 (+0,8~\%) \\
CORAL supervisé (20~\% labels) & \textbf{0,629 ($\times 15$)} \\
\hline
\end{tabularx}
\caption{Résultats adaptation HAPT $\rightarrow$ WISDM}
\label{tab:coral_impl}
\end{table}

Le gain spectaculaire du CORAL supervisé (+×15 par rapport à l'absence d'adaptation) s'explique par l'effet synergique de l'alignement de covariance et des quelques exemples étiquetés : l'alignement crée une représentation commune source-cible, puis la supervision affine la tête de classification dans cet espace aligné.
...
d'anticipation LSTM, détecteur de chutes hybride, adaptateur CORAL — est un
module Python indépendant intégré dans \texttt{HARContinualModel}. Les expériences
valident l'augmentation de données, l'UWR et CORAL ; l'anticipation multi-classe
reste difficile. Le chapitre suivant présente l'évaluation systématique.
...
SSL / foundation-style                           & \texttt{ssl\_backbone.pt}, \texttt{foundation\_style.pt} \\
...
Sur le protocole fall-risk (\texttt{train\_fall\_risk.py}), le macro-F1 de
validation atteint 0,88 pour $p\in\{50\%,75\%\}$ (accuracy $\approx$0,925).
Le SSL et le pipeline foundation-style fournissent des initialisations
optionnelles ; la calibration et le bandit de seuil sont validés
fonctionnellement dans Streamlit.
...
\begin{itemize}
  ...
  (MobiAct) ; repas non entraînable sans corpus cuisine.
...
\textbf{Validation chutes.} Intégrer MobiAct et comparer règles seules, ML seul
et hybride sur chutes réelles clôturerait la boucle aide à domicile évoquée au
chapitre~1.
...
\textbf{UWR.} Le tampon de rejeu priorise les exemples incertains (MC Dropout).
...
\textbf{CORAL.} Aligner les covariances d'embeddings avec 20~\% de labels WISDM
multiplie le F1 cross-dataset par~15 (0,040 $\rightarrow$ 0,629).
...
Adaptation CORAL (F1)      & 0,040   & 0,629   & $\times 15$ \\
...
contraintes réalistes. UWR et CORAL apportent des gains mesurables ;
...
\begin{itemize}
  ...
  \item \textbf{Cross-dataset CORAL} : atteint (F1 0,629, $\times 15$) avec 20~\%
  labels cibles.
...
épistémique MC sur IMU (UWR) ; (2)~CORAL appliqué aux embeddings d'un
...
labels cibles — CORAL seul ne suffit pas. Ces enseignements sont documentés
...
Poids UWR (incertitude) & MC Dropout, $T=20$ échantillons \\
...
\hline
CORAL supervisé HAPT$\rightarrow$WISDM & F1 macro & 0,629 \\
...
# Entrainement : minimiser coral_loss + CE sur 20% labels cibles
\end{lstlisting}

En pratique, le gain $\times 15$ observé au chapitre~6 vient surtout du
...
\begin{lstlisting}[language=Python,caption={Estimation entropique (\texttt{mc\_dropout.py})},label=lst:mc]
...
\textbf{Lien mémoire.} Module \texttt{CORALAdapter} ; F1 0,629 avec 20~\% labels
WISDM chapitre~6.
...
\hline
\textbf{Macro} & \textbf{0,040} & \textbf{0,629} & \textbf{+0,589} \\
\hline
\end{tabularx}
