...
backbone, un tampon de rejeu de 2\,000 fenêtres, et des modules d'adaptation de domaine, SSL,
...
le fall-risk binaire ($\approx$0,88), et l'adaptation cross-dataset (\texttt{DomainAdapter})
(F1 0,46). Le scénario « préparation de repas » reste hors portée faute de
...
tête LSTM sur fenêtres partielles, plus des modules d'adaptation de domaine, SSL,
...
\begin{enumerate}
  ...
  adaptateur de domaine (\texttt{DomainAdapter}), fall-risk, calibration online, bandit de seuil,
...
(C1) Uncertainty-Weighted Replay (UWR) : stratégie de sélection et d'échantillonnage d'exemples dans le tampon de rejeu basée sur l'entropie de la prédiction softmax (une passe, paramètre $\alpha$). À notre connaissance, c'est la première application de cette stratégie au contexte HAR continu.
...
\begin{tikzpicture}[
  ...
  \node[mod, below=0.55cm of ant] (coral) {Domain\\Adapter};
  \draw[arr] (imu) -- (prep);
  \draw[arr] (prep) -- (bb);
  \draw[arr] (bb) -- (cl);
  \draw[arr] (bb) -- (ant);
...
(entropie softmax) pour guider le tampon de rejeu. L'anticipation
...
Il n'existe pas de fichier \texttt{uncertainty\_replay.py} dans le dépôt livré : le MC Dropout reste une référence bibliographique, pas l'implémentation utilisée pour pondérer le rejeu.
...
classe \texttt{DomainAdapter} dans le code.
...
complet ; les anciens tableaux « CORAL 0,46 / domain shift » ne correspondent
...
d'anticipation LSTM, détecteur de chutes hybride, adaptateur de domaine (\texttt{DomainAdapter}) — est un
